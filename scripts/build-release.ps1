...
# Get script directory and determine root
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path

# Auto-detect if script is in "scripts" subfolder
...
# Verify solution file exists in root
$SolutionFile = Join-Path $RootDir "NestLaserDesktop.sln"
$ProjectFile = Join-Path $RootDir "NestLaserDesktop.csproj"
...
# For portable build, restore with RuntimeIdentifier (must use project, not solution)
if ($Portable) {
...
if ($Portable) {
    # Portable builds must use project file with RuntimeIdentifier and self-contained
    dotnet build $ProjectFile -c Release -v q -r win-x64 --self-contained true --no-restore
} else {
...
# Step 4: Test
if ($Portable) {
    Write-Host "[SKIP] Tests skipped for portable build (RID mismatch)" -ForegroundColor Yellow
} elseif (-not $SkipTests) {
    Write-Step "Step 4: Running tests"
    # Tests don't need RuntimeIdentifier, use solution
...
# Output location
if ($Portable) {
    $ExePath = "dist\portable\NestLaserDesktop.exe"
} else {
    $ExePath = "dist\release\NestLaserDesktop.exe"
}
