...
| Unit handling | Assumes mm → now parses $INSUNITS/$MEASUREMENT/$LUNITS and scales accordingly | Correct scaling to mm | ✅ Fixed in FAZ 9X | Low ✅ Fixed |
...
| Area | Current Behavior | Expected Behavior | Issues | Priority |
|------|------------------|-------------------|--------|----------|
| Quotation PDF | PdfReportService writes native PDF | Customer-ready quotation output | — | — |
| Production PDF | Uses same project/cost data with production report title | Shop-floor/customer production report | — | — |
| Preview | Plate and placed geometry drawn as vector paths | Customer sees nesting layout in PDF | — | — |
| Company header | CompanyProfile fields shown, LogoPath embedded when readable | Branded output | Unsupported/missing logo file is ignored safely | Low |
| Project persistence | CompanyProfile and PdfReportSettings stored in `.nelp` | Settings survive save/load | — | — |
| Validation | Warns for missing project, nesting, material, machine, cost | Prevent incomplete PDF | — | — |
...
### Low: DXF unit handling — assumes mm without header parsing
- **Files:** `Models/ImportUnitInfo.cs`, `Services/DxfHeaderParser.cs`, `Services/DxfService.cs`
- **Problem:** `DxfService.Import()` read geometry coordinates directly without checking the DXF HEADER for `$INSUNITS`, `$MEASUREMENT`, or `$LUNITS`. If the source CAD used inches, cm, or any unit other than mm, all dimensions would be wrong. Users had no way to correct import scale.
- **Fix (FAZ 9X):** Created `ImportUnitInfo` model with source/target unit and scale factor. Created `DxfHeaderParser.DetectUnit()` that reads DXF HEADER variables using a stream reader. `DxfService.Import()` now accepts optional `ImportUnitInfo` override; auto-detects if not provided. Vertex coordinates are scaled by `ScaleFactorToMm` before polygon construction. Added `ImportUnitInfo`, `TotalBoundingWidth`, `TotalBoundingHeight` to `DxfImportResult`. Added manual scale override and reference-dimension-based scale suggestion in the new Doğrulama tab.
- **Test:** Import a DXF with $INSUNITS=1 (Inches); all geometry is multiplied by 25.4. Import a unitless DXF; warning shown, user can apply manual scale. Enter reference dimension in the verification panel; suggested scale factor computed correctly.
