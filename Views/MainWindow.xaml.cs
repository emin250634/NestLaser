...
{
    private static readonly SolidColorBrush GridBrush = FreezeBrush(Color.FromArgb(15, 0x4A, 0x52, 0x5A));
    private static readonly SolidColorBrush MajorGridBrush = FreezeBrush(Color.FromArgb(35, 0x4E, 0xC9, 0xB0));
    private static readonly SolidColorBrush TickBrush = FreezeBrush(Color.FromRgb(0x6A, 0x75, 0x80));
    private static readonly SolidColorBrush RulerTextBrush = FreezeBrush(Color.FromRgb(0xB0, 0xB8, 0xC0));
    private static readonly SolidColorBrush MajorTickBrush = FreezeBrush(Color.FromRgb(0x4E, 0xC9, 0xB0));
    ...
    private static readonly SolidColorBrush HoverBrush = FreezeBrush(Color.FromArgb(200, 0x4E, 0xC9, 0xB0));
    private static readonly SolidColorBrush PlateFillBrush = FreezeBrush(Color.FromRgb(0x18, 0x1A, 0x1C));
    private static readonly SolidColorBrush PlateStrokeBrush = FreezeBrush(Color.FromRgb(0x4E, 0xC9, 0xB0));
    private static readonly SolidColorBrush MarginStrokeBrush = FreezeBrush(Color.FromArgb(60, 0x4E, 0xC9, 0xB0));
    private static readonly SolidColorBrush PlateLabelBrush = FreezeBrush(Color.FromRgb(0x90, 0x95, 0x9A));
    ...
    private static readonly SolidColorBrush SelectionFillBrush = FreezeBrush(Color.FromArgb(40, 0x4E, 0xC9, 0xB0));
    private static readonly SolidColorBrush SelectionStrokeBrush = FreezeBrush(Color.FromArgb(200, 0x4E, 0xC9, 0xB0));
    ...
    private static double GetNestedPlateOffsetX(NestResult result, int plateIndex)
    {
        const double gapMm = 20;
        double offset = 0;
        for (int i = 0; i < plateIndex && i < result.Plates.Count; i++)
            offset += result.Plates[i].Width + gapMm;
        return offset;
    }
    ...
    private void UpdateViewportBounds()
    {
        ...
        _viewportMinX = Wx(0);
        _viewportMaxX = Wx(canvasW);
        _viewportMinY = Wx(0);
        _viewportMaxY = Wx(canvasW);
    }
    ...
    private bool IsVisibleInViewport(Geometry.BoundingBox bounds)
    {
        if (bounds.MinX > bounds.MaxX || bounds.MinY > bounds.MaxY)
            return false;
        return bounds.MinX <= _viewportMaxX && bounds.MaxX >= _viewportMinX &&
               bounds.MinY <= _viewportMaxY && bounds.MaxY >= _viewportMinY;
    }
    ...
    private void MainWindow_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        LayerColorConverter.SetViewModel(_vm);
        if (_vm != null)
        {
    ...
    private SnapDelta ResolveGridSnap(double baseDx, double baseDy)
    {
        ...
        var anchor = _vm.GetSelectionBoundsPublic();
    ...
    private SnapDelta ResolvePreciseSnap(double baseDx, double baseDy)
    {
        ...
        var selected = _vm.SelectedParts.Select(p => p.Id).ToHashSet();
        var targets = _vm.Parts.Where(p => !selected.Contains(p.Id) && _vm.IsPartVisible(p)).ToList();
    ...
    private SnapDelta ResolveVertexSnap(double baseDx, double baseDy, List<PartModel> targets, double toleranceMm)
    {
        ...
        {
            var sourceGeometry = _vm.NestResult != null && _vm.NestResult.Placed.Count > 0
                ? _vm.NestResult.Placed.FirstOrDefault(p => p.PartId == selectedPart.Id)?.TransformedGeometry ?? selectedPart.Geometry
                : selectedPart.Geometry;

            foreach (var sourceVertex in sourceGeometry.Vertices)
            {
                var moved = new Geometry.Point2D(sourceVertex.X + baseDx, sourceVertex.Y + baseDy);
                foreach (var target in targets)
                {
                    foreach (var targetVertex in GetTargetGeometry(target).Vertices)
                    {
    ...
    private SnapDelta ResolveEdgeSnap(double baseDx, double baseDy, List<PartModel> targets, double toleranceMm)
    {
        ...
        foreach (var selectedPart in _vm.SelectedParts)
        {
            var b = (_vm.NestResult != null && _vm.NestResult.Placed.Count > 0
                ? _vm.NestResult.Placed.FirstOrDefault(p => p.PartId == selectedPart.Id)?.TransformedGeometry.Bounds
                : null) ?? selectedPart.Geometry.Bounds;
            var movedX = new[] { b.MinX + baseDx, b.MaxX + baseDx };
            var movedY = new[] { b.MinY + baseDy, b.MaxY + baseDy };

            foreach (var target in targets)
            {
                var tb = GetTargetGeometry(target).Bounds;
    ...
    private SnapDelta ResolveCenterSnap(double baseDx, double baseDy, List<PartModel> targets, double toleranceMm)
    {
        ...
        foreach (var selectedPart in _vm.SelectedParts)
        {
            var center = (_vm.NestResult != null && _vm.NestResult.Placed.Count > 0
                ? _vm.NestResult.Placed.FirstOrDefault(p => p.PartId == selectedPart.Id)?.TransformedGeometry.Bounds.Center
                : null) ?? selectedPart.Geometry.Bounds.Center;
            double movedCenterX = center.X + baseDx;
            double movedCenterY = center.Y + baseDy;

            foreach (var target in targets)
            {
                var targetCenter = GetTargetGeometry(target).Bounds.Center;
    ...
    private Geometry.Polygon GetTargetGeometry(PartModel target)
    {
        if (_vm?.NestResult != null && _vm.NestResult.Placed.Count > 0)
        {
            var placement = _vm.NestResult.Placed.FirstOrDefault(p => p.PartId == target.Id);
            if (placement != null)
                return placement.TransformedGeometry;
        }
    ...
    private void DrawPreview()
    {
        ...
        try
        {
            ...
            else if (_vm.Parts.Count > 0)
            {
                ...
                if (_vm.Parts.Count > 0 && _renderedPartCount == 0)
                {
                    var firstPart = _vm.Parts[0];
                    var bounds = firstPart.Geometry.Bounds;
                    _vm.StatusText = $"DEBUG: Parts={_vm.Parts.Count} Rendered={_renderedPartCount} Culled={_culledPartCount} Bounds=({bounds.MinX},{bounds.MinY})-({bounds.MaxX},{bounds.MaxY}) Plate={_vm.Plate.Width}x{_vm.Plate.Height} Zoom={_zoom:F2}";
                }
            }
    ...
    private void DrawMultiPlateNesting(NestResult result)
    {
        ...
        for (int i = 0; i < result.Plates.Count; i++)
        {
            ...
            for (int j = 0; j < platePlacements.Count; j++)
            {
                ...
                // Skip culling - render all placements (viewport culling had coordinate issues)
                _renderedPartCount++;
    ...
    private void DrawPartsOnPlate(ObservableCollection<PartModel> parts, PlateModel plate)
    {
        ...
        int skippedNoVertices = 0;

        foreach (var part in parts)
        {
            if (_vm != null && !_vm.IsPartVisible(part))
            {
                skippedInvisible++;
                continue;
            }
            if (part.Geometry.Vertices.Count < 2)
            {
                skippedNoVertices++;
                continue;
            }
            ...
            // Skip culling - render all parts (viewport culling had coordinate system issues)
            // var bounds = part.Geometry.Bounds;
            // double displayMinY = plate.Height - bounds.MaxY;
            // double displayMaxY = plate.Height - bounds.MinY;
            // if (!IsVisibleInViewport(bounds.MinX, bounds.MaxX, displayMinY, displayMaxY))
            ...
            DrawScreenPolygon(screenVertices, fillColor, new SolidColorBrush(color), 2.0);
            colorIndex++;
        }

        // All parts rendered - culling disabled due to coordinate issues
    }
    ...
    private void DrawBoundingBoxes()
    {
        ...
        if (_vm.NestResult != null && _vm.NestResult.Plates.Count > 0)
        {
            ...
            for (int plateIdx = 0; plateIdx < _vm.NestResult.Plates.Count; plateIdx++)
            {
                var plate = _vm.NestResult.Plates[plateIdx];
                if (plate == null) continue;

                foreach (var p in _vm.NestResult.Placed.Where(pl => pl.PlateIndex == plateIdx))
                {
                    if (!_vm.IsPartVisible(p.Part)) continue;
                    if (p.TransformedGeometry.Vertices.Count < 2) continue;

                    DrawWorldPolyline(p.TransformedGeometry.Vertices, plate.Height, currentWorldX,
                        new SolidColorBrush(Color.FromArgb(60, 0xFF, 0xFF, 0x00)), 0.8);
                }
                currentWorldX += plate.Width + gapMm;
            }
        }
        else if (_vm.Parts.Count > 0)
        {
            ...
            foreach (var part in _vm.Parts)
            {
                ...
                DrawScreenPolyline(screenVertices, new SolidColorBrush(Color.FromArgb(60, 0xFF, 0xFF, 0x00)), 0.8);
            }
        }
    }
	...
	private void DrawSelectedPartHighlights()
	{
        ...
        if (_vm.NestResult != null && _vm.NestResult.Plates.Count > 0)
        {
            for (int plateIdx = 0; plateIdx < _vm.NestResult.Plates.Count; plateIdx++)
            {
                var plate = _vm.NestResult.Plates[plateIdx];
                if (plate == null) continue;
                double plateOffsetX = GetNestedPlateOffsetX(_vm.NestResult, plateIdx);
                ...
                foreach (var sel in _vm.SelectedParts)
                {
                    ...
                    foreach (var p in _vm.NestResult.Placed.Where(pl => pl.PlateIndex == plateIdx))
                    {
                        if (p.PartId != sel.Id) continue;
                        if (p.TransformedGeometry.Vertices.Count < 2) continue;

                        DrawWorldPolyline(p.TransformedGeometry.Vertices, plate.Height, plateOffsetX, HighlightBrush, 2.0);
                        DrawCornerHandles(p.TransformedGeometry.Vertices, plate.Height, plateOffsetX);
                        break;
                    }
                }
            }
    ...
    private void DrawHoverPartHighlight()
    {
        ...
        if (_vm.NestResult != null && _vm.NestResult.Plates.Count > 0)
        {
            foreach (var plate in _vm.NestResult.Plates.Select((p, i) => new { Plate = p, Index = i }))
            {
                double plateOffsetX = GetNestedPlateOffsetX(_vm.NestResult, plate.Index);
                foreach (var p in _vm.NestResult.Placed.Where(pl => pl.PlateIndex == plate.Index))
                {
                    if (p.PartId != _hoveredPart.Id || p.TransformedGeometry.Vertices.Count < 2) continue;
                    DrawWorldPolyline(p.TransformedGeometry.Vertices, plate.Plate.Height, plateOffsetX, HoverBrush, 2.0);
                    return;
                }
            }
        }
