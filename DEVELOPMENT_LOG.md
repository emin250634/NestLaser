...
## [2026-06-13] FAZ 9B - True Shape Nesting Debug & Gap Fill Rework
- `NestResult` içine TrueShape debug sayaçları eklendi: candidate, vertex-to-vertex, vertex-to-edge, edge-to-edge, gap-fill attempt/success, boundary/bbox/SAT rejection ve accepted sayıları.
- Analysis tab'a `True Shape Debug` özeti eklendi; sayaçlar doğrudan görünür hale geldi.
- TrueShape aday yerleştirme akışı anchor-temelli translation'a geçirildi: `translation = targetPoint - candidateAnchor`.
- Gap fill için polygon bounding box içi sample noktalar eklendi; küçük parçalar gerçek boşluklara tekrar deneniyor.
- Yeni regression testleri eklendi: hexagon gap, concave L gap, anchor correctness, gap-fill counter coverage.
- Amaç: gerçek DXF'te TrueShape farkı üretmiyorsa nedeni sayısal olarak ayırabilmek.

## [2026-06-14] BUGFIX 9B.2 - Real Gap Fill + Multi-Plate Overlay Fix
- Nested overlay çizimleri artık doğru plaka ofseti ile hesaplanıyor; selection ve hover outline ikinci plakada da doğru konuma gidiyor.
- TrueShape gap fill, usable plate alanında düşük çözünürlüklü serbest alan örnekleri alacak şekilde genişletildi; küçük parçalar mevcut plaka boşluklarına yeni plaka açmadan önce tekrar deneniyor.
- Küçük parçalar için center/centroid/bounding-box corner/vertex anchor modları ve 0/90/180/270 rotasyon adayları korunup güçlendirildi.
- Debug sayaçlarına small-part gap candidates/success, same-plate gap success, and new-plate avoided count eklendi.
- Yeni regression testleri: multi-plate overlay offset, hexagon+circle same-plate gap reuse, rotated rectangle placement.

## [2026-06-14] BUGFIX 9B.3 - Snap & Move Coordinate Fix
- Nested snap/drag akışı artık `Part.Geometry` yerine yerleşmiş `TransformedGeometry` ve selection bounds üzerinden çalışıyor.
- Taşıma, rotate ve mirror komutları nested placement'ları `PartId` ile eşleştirip aynı anda güncelliyor.
- Aynı isimli parçalar için yanlış placement eşleşmesi kaldırıldı.
- Yeni regression testleri nested selection bounds ve placement move güncellemesini kapsıyor.

## [2026-06-14] BUGFIX 9B.4 - Cross-Plate Rehome
- Bir parça drag ile başka plaka bandına geçtiğinde placement artık aynı ekranda kalacak şekilde yeni PlateIndex'e yeniden bağlanıyor.
- Böylece parça ikinci plakaya taşındıktan sonra kaybolmuyor ve tekrar seçilebilir/taşınabilir kalıyor.
- Move regression testi plate boundary crossing senaryosunu kapsıyor.
