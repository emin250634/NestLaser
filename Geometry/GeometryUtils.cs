...
public static class GeometryUtils
{
    ...
    public static bool PolygonsIntersectDetailed(Polygon a, Polygon b, out double overlapDepth, out Point2D overlapAxis, double epsilon = 1e-9)
    {
        overlapDepth = double.MaxValue;
        overlapAxis = new Point2D(0, 0);
        ...
        foreach (var axis in axes)
        {
            ...
            double axisOverlap = Math.Min(maxA - minB, maxB - minA);
            if (axisOverlap < overlapDepth)
            {
                overlapDepth = axisOverlap;
                overlapAxis = axis;
            }
        }
