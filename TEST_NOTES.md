...
- TrueShape debug counters were added to `NestResult` and surfaced in the Analysis tab.
- The Analysis tab now shows a compact `True Shape Debug` report string with candidate counts, gap-fill attempts, rejection counts, and fallback state.
- New tests cover hexagon bounding-box void placement, concave L-shape gap fill, anchor translation correctness, and gap-fill attempt/success counters.
- Phase 9B.2 adds regression coverage for multi-plate overlay offsets, hexagon+circle same-plate gap reuse, and 90-degree rotation placement.
- The debug report now also exposes small-part gap sampling, same-plate gap success, and new-plate avoidance counters.
- Phase 9B.3 adds nested snap/move regression coverage using transformed placement geometry and `PartId`-based placement updates.
- Phase 9B.4 covers cross-plate rehome behavior so a part moved from plate 1 to plate 2 remains selectable and keeps the correct plate index.
